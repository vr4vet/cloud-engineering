    ...
    public class TaskManager : MonoBehaviour
    {
        ...
        private void CreateOppgavePage(Task oppgave)
        {
            ...
            foreach (Activity aktivitetObject in oppgave.GetAktivitetList())
            {
                ...
                aktivitet.transform.Find("Poeng").GetComponent<Text>().text = aktivitetObject.maxPosiblePoints == 0 ? string.Empty : (myPoeng + "/" + totalPoengStr); //aktivitet name

                //Manage the checkbox
                GameObject checkbox = aktivitet.transform.Find("Checkbox").gameObject;
                if (totalAchievedPoeng == 0 && !aktivitetObject.AktivitetIsCompeleted)
                {
